...
- `contact_messages` para que un formulario sin SMTP no pierda leads.
...
- `tenants.contact_email` para formularios de contacto; no usar `SMTP_TO_EMAIL`.
...
- La migración de triggers ISR **siempre debe incluir `tenants`**. Sin el trigger ISR en `tenants`, los cambios de configuración del negocio (MP, SMTP, WhatsApp, nombre) no invalidan el cache.

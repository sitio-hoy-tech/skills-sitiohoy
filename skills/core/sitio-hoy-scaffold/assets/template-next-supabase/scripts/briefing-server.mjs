...
function publicTenantSnapshot(row, related = {}) {
  return {
    ...
    smtpConfigured: Boolean(row.smtp_user),
...
function mergeTenantIntoIntake(intake, tenant) {
  ...
  intake.technical = {
    ...
    smtpRequested: hasCheckout && (Boolean(tenant.smtp_user) || Boolean(intake.technical?.smtpRequested)),
    domain: {
...
async function handleSubmit(req, res) {
  ...
  const newIntegrations = {
    ...
    smtp: hasCheckout && Boolean(tech.smtpRequested),
    umami: hasCheckout,
    whatsapp: true,
  }
  ...
  const tenantLookupLines = isExisting
    ? [
        ...
        ...(tenant ? [
          ...
          `- SMTP configurado: ${yesNo(tenant.smtpConfigured)}`,
